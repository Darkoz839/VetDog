...
<?php
  ...
  $dbHost = "localhost";
  $dbDatabase = "id21966086_vetdog";
  $dbPasswrod = "257780Lu$";
  $dbUser = "id21966086_vetdog";
        ...
        <h2>
          ...
          <div class="col-sm-4">
            ...
            <div class="form-group">
                <div class="form-line">
                  <h2>Subir hasta 4 fotos de perros:</h2>
                    <input type="file" id="fileInput1" accept="image/*"><br>
                    <input type="file" id="fileInput2" accept="image/*"><br>
                    <input type="file" id="fileInput3" accept="image/*"><br>
                    <input type="file" id="fileInput4" accept="image/*"><br>
                    <button onclick="uploadImages()">Subir imágenes</button><br><br>
...
<!-- Jquery Core Js -->
   ...
   <script>
        ...
        async function uploadImages() {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                img.onload = async function() {
                    ...
                    displayResults(predictions, i + 1);
                };
        ...
        async function modelPredict(tensor) {
            ...
            const predictions = await modelo.predict(tensor).array();
            return predictions;
        }

        function displayResults(predictions, index) {
            const enfermedades = ['Alergia a Pulgas', 'Hotspo', 'Sarna', 'Dermatofitosis'];
            const resultadosDiv = document.getElementById('resultados');

            resultadosDiv.innerHTML += '<h3>Resultados para imagen ' + index + ':</h3>';

            // Mostrar las 4 posibles enfermedades con sus porcentajes de precisión
            for (let i = 0; i < 4; i++) {
                const porcentaje = predictions[0][i] * 100;
                resultadosDiv.innerHTML += '<p>' + enfermedades[i] + ': ' + porcentaje.toFixed(2) + '%</p>';
            }
        }
    </script>
